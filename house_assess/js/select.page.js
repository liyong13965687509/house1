...
SelectPage.prototype.ishistory = function () {
   ...
   //如果有，转换成 数组的形式存放到searchArr的数组里（localStorage以字符串的形式存储，所以要把它转换成数组的形式）
        var optionss = localStorage.getItem('search');
...
SelectPage.prototype.close = function () {
    $("#plot-list").on("click", "li img", function () {
        ...
        for (var i = 0; i < searchArr.length; i++) {
            ...
            // 将新的数组存到localstorage  更新localstorage
            localStorage.search = JSON.stringify(searchArr);
        }
    })
    return this;
}
...
SelectPage.prototype.selected = function () {
    ...
    $("#plot-list").on("click", "li a", function () {
        localStorage.setItem('plot_name', $(this).html().trim());
        // 存储选中的小区名称和row_id
        var plot = {
