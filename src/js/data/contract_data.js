    ...
    switch (name) {
        case this.CTN:
            params = {
                ...
                url: host + "/contract/condition"
            };
            break;
        case this.CPI:
            params = {
                url: host + "/contract/contracts",
                requestKey: localStorage.getItem("requestKey"),
                // 当前页码
                pageIndex: 1,
                // 一页显示条数
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data['succ']) {
                // ContractInit();
                // cp.init();
                // Contract($("#CurContractCharId").val());
                cp.ajaxRequestContractDetail(cp.getParams(cp.CTT));
                messageBox.show("提示", data['msg'], MessageBoxButtons.OK, MessageBoxIcons.infomation);
                baocun();
            }
