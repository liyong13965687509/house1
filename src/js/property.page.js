...
function PropertyPage() {
    ...
    this.API_CONFIG = arguments['API_CONFIG'] ? arguments['API_CONFIG'] : {
        ...
        HISTORY_ADDCONTRACTS: '/contract/historyadd',
 ...
 * @returns {boolean}
 */
PropertyPage.prototype.historyContractAddNotEmpty = function () {
    var result = false;
    var historyMessage = "";
    if ('' == $('#historyCustomerName_Add').val().trim()) {
        historyMessage = "请选择签约客户";
    } else if (!regular.PHONE_REG_EXP.test($('#historyContractPhone_Add').val().trim())) {
        historyMessage = "手机号码不正确";
    } else if ('' == $('#historyCardID_Add').val().trim()) {
        historyMessage = "证件信息不能为空";
    } else if ('' == $('#HistoryAddContract_Number').val().trim()) {
        historyMessage = "合同编号不能为空";
    } else if (!regular.MONEY_REG_EXP.test($('#HistoryAddContract_Price').val().trim())) {
        historyMessage = "租金输入有误";
    } else if (!regular.MONEY_REG_EXP.test($('#HistoryAddContract_Deposit').val().trim())) {
        historyMessage = "押金输入有误";
    } else if ('' == $('#HistoryAddContract_InDate').val().trim()) {
        historyMessage = "请填写起租日期";
    } else if ('' == $('#HistoryAddContract_OutDate').val().trim()) {
        historyMessage = "请填写退租日期";
    } else if (!regular.CUSTOMER_REG_EXP.test($('#HistoryAddContract_Persons').val().trim())) {
        historyMessage = "入住人数输入有误";
    } else if (!$('#HistoryAddContract_Dpts .active').attr('data-value')) {
        historyMessage = "请选择员工部门";
    } else if (!$('#HistoryAddContract_Employee .active').attr('data-value')) {
        historyMessage = "请选择员工";
    } else if ('' == $('#HistoryBargainDate').val().trim()) {
        historyMessage = "请填写签约日期";
    } else if ('' == $('#HistoryEndDate').val().trim()) {
        historyMessage = "请填写退租日期";
    } else {
        result = true;
    }
 ...
 * @returns {PropertyPage}
 */
PropertyPage.prototype.ajaxRequestHistoryAddContract = function (params) {
    var _this = this;
    $.ajax({
        type: "POST",
        url: host + _this.API_CONFIG.HISTORY_ADDCONTRACTS,
        data: params,
        dataType: "JSON",
 ...
 * @returns {PropertyPage}
 */
PropertyPage.prototype.exeHistoryAddContract = function () {
    if (this.historyContractAddNotEmpty()) {
        var params = this.getParams(this.API_CONFIG.HISTORY_ADDCONTRACTS);
        this.ajaxRequestHistoryAddContract(params)
    }
    return this;
}
/**
 ...
 * @returns {PropertyPage}
 */
PropertyPage.prototype.showHistoryContactAdd = function () {
    // this.COMM_VALUE = $(this).attr('data-value').trim();
    var _this = this;
    localStorage.setItem('StatusCode', '2');
    mp.manualShowPanel({
        index: 9,
        element: '.panel-sm',
        complete: function () {
...
PropertyPage.prototype.appendHtmlBindEmployee = function (data) {
    ...
    $('#HistoryAddContract_Employee').html(TEMP_HTML);
    return this;
}
/**
 * BEGIN 绑定物业列表
...
PropertyPage.prototype.exeBindEmployee = function () {
    var SELECTOR = '.panel-modal.show .tree-menu:eq(0) .active';
    this.COMM_VALUE = $(SELECTOR).attr('data-value').trim();
...
PropertyPage.prototype.appendHtmlBindContract = function (data) {
    ...
    $('#HistoryAddContract_Number').val(JSON_DATA['ContractNum']);
    ...
    $('#HistoryAddContract_Payment').html(TEMP_HTML);
    $('#HistoryAddContract_PaymentResult').text($('#HistoryAddContract_Payment .active').text().trim());
    ...
    $('#HistoryAddContract_BuildingName').val(TEMP_BUILD);
    $('#HistoryAddContract_Layout').val(TEMP_LAYOUT);
    $('#HistoryAddContract_Square').val(JSON_DATA['Square']);
    return this;
}
/**
 * BEGIN 绑定物业列表
...
PropertyPage.prototype.getParams = function (name) {
    ...
    switch (name) {
        ...
        case this.API_CONFIG.BIND_EMPLOYEE:
            ...
            break;
        case this.API_CONFIG.HISTORY_ADDCONTRACTS:
            params = {
                requestKey: localStorage.getItem("requestKey"),
                number: $("#HistoryAddContract_Number").val().trim(),
                phone: $("#historyContractPhone_Add").val().trim(),
                cardID: $("#historyCardID_Add").val().trim(),
                buildingRoomCharId: _this.DATA_VALUE,
                customerCharId: $("#historyCustomerCharId").val().trim(),
                price: parseFloat($("#HistoryAddContract_Price").val().trim()),
                deposit: parseFloat($("#HistoryAddContract_Deposit").val().trim()),
                payType: parseInt($("#HistoryAddContract_Payment .active").attr("data-value")),
                inDate: $("#HistoryAddContract_InDate").val().trim(),
                outDate: $("#HistoryAddContract_OutDate").val().trim(),
                total: parseInt($("#HistoryAddContract_Persons").val().trim()),
                bargainDate: $("#HistoryBargainDate").val().trim(),
                endDate: $("#HistoryEndDate").val().trim(),
                ownerDepartmentCharId: $("#HistoryAddContract_Dpts .active").attr("data-value"),
                ownerEmployeeCharId: $("#HistoryAddContract_Employee .active").attr("data-value"),
                description: $("#HistoryAddContract_Description").val().trim()
            }
            break;
    }
    return params;
