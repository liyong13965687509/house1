...
function DateComponent() {
    var arguments = arguments.length != 0 ? arguments[0] : arguments;
    this.element = arguments['element'] ? arguments['element'] : '.date';
    this.beginDate = arguments['beginDate'] ? arguments['beginDate'] : '';
    this.cycleDate = arguments['cycleDate'] ? arguments['cycleDate'] : '';
    this.endDate = arguments['endDate'] ? arguments['endDate'] : '';
 ...
 */
DateComponent.prototype.init = function () {
    this.dateTime();
    this.clickCycleDate();
    return this;
}
 ...
 * @returns {DateComponent}
 */
DateComponent.prototype.dateTime = function () {
    var TEMP_DATE = new Date();
    var TEMP_YEAR = TEMP_DATE.getFullYear();
    var TEMP_MONTH = TEMP_DATE.getMonth() + 1;
    var TEMP_DAY = TEMP_DATE.getDate();
    var TEMP_STR = TEMP_YEAR + '-' + TEMP_MONTH + '-' + TEMP_DAY;
    $(this.element).val(TEMP_STR);
    TEMP_DATE = null;
    return this;
}
/**
 ...
 * @returns {DateComponent}
 */
DateComponent.prototype.clickCycleDate = function () {
    var _this = this;
    $(document).on('click', this.cycleDate, function () {
        var DATA_LENGTH = $(this).attr('data-length');
        _this.endDate = '#' + $(this).attr('data-target');
        var BEGIN_DATE = $(_this.beginDate).val().trim();
        var TEMP_ARRAY = BEGIN_DATE.split('-');
        TEMP_ARRAY[1] = parseInt(TEMP_ARRAY[1]) + parseInt(DATA_LENGTH);
        TEMP_ARRAY[0] = TEMP_ARRAY[1] > 12 ? (parseInt(TEMP_ARRAY[0]) + 1) + '' : TEMP_ARRAY[0];
        TEMP_ARRAY[1] = TEMP_ARRAY[1] > 12 ? (TEMP_ARRAY[1] - 12) : TEMP_ARRAY[1];
        TEMP_ARRAY[1] = TEMP_ARRAY[1] > 9 ? TEMP_ARRAY[1] + '' : '0' + TEMP_ARRAY[1];

        //计算时间戳
        var NEW_DATE = TEMP_ARRAY[0] + '-' + TEMP_ARRAY[1] + '-' + TEMP_ARRAY[2];
        var NEW_DATE_TIME = new Date(NEW_DATE).getTime() - (1 * 24 * 60 * 60 * 1000);

        $(_this.endDate).val(webApp.parseDate(NEW_DATE_TIME).substring(0, 11));
    });
    return this;
}
/**
 ...
 * @type {DateComponent}
 */
var dateComponent = new DateComponent({
    beginDate: '#AddContract_InDate',
    endDate: '#AddContract_OutDate',
    cycleDate: '.radio-item>label'
});